...
- **Retry limits.** The first unrecognized token-shaped credential from each source IP in a bridge run is treated as a stale token and does not spend a strike, on either transport. After that, the first 3 wrong pairing-code guesses have no delay. Later misses sleep for 2 seconds, 4 seconds, then at most 8 seconds per attempt. The hard limit is 10 code guesses per source IP per bridge run. Strike counts survive reconnects during that run.
